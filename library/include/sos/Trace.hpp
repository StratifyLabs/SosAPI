...
public:
  ...
  u32 program_address() const {
    return m_event.posix_trace_event.posix_prog_address;
  }
...
public:
  ...
  Trace &operator=(const var::String &a) {
    var::String::operator=(a);
    return *this;
  }
  /*! \details Sends trace as a message. */
  inline void trace_message() MCU_ALWAYS_INLINE {
    sos_trace_event(LINK_POSIX_TRACE_MESSAGE, cstring(), length());
  }
  /*! \details Sends trace as a warning. */
  inline void trace_warning() MCU_ALWAYS_INLINE {
    sos_trace_event(LINK_POSIX_TRACE_WARNING, cstring(), length());
  }
  /*! \details Sends trace as an error. */
  inline void trace_error() MCU_ALWAYS_INLINE {
    sos_trace_event(LINK_POSIX_TRACE_ERROR, cstring(), length());
  }
  /*! \details Sends trace as a critical message. */
  inline void trace_critical() MCU_ALWAYS_INLINE {
    sos_trace_event(LINK_POSIX_TRACE_CRITICAL, cstring(), length());
  }
  /*! \details Sends trace as a fatal message.
   *
   * Trace fatal will not cause the program to exit but for good practice, you
   * must invoke exit() immediately after a trace_fatal() call.
   *
   */
